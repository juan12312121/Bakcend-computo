...
class Usuario {
    ...
    try {
      const query = `
        SELECT 
          ...
          total_siguiendo, total_posts, activo, suspendido, rol,
          intentos_fallidos, bloqueo_hasta, fecha_creacion
        FROM usuarios 
  ...
  static async registrarIntentoFallido(id) {
    try {
      const query = `
        UPDATE usuarios 
  ...
  // 🔐 SEGURIDAD: Bloquear cuenta temporalmente
  static async bloquearCuenta(id, minutos = 30) {
    try {
      const bloqueoHasta = new Date(Date.now() + minutos * 60000);
      const query = `
        UPDATE usuarios 
        SET bloqueo_hasta = ?
        WHERE id = ?
      `;
      const [resultado] = await db.execute(query, [bloqueoHasta, id]);
      return resultado.affectedRows > 0;
    } catch (error) {
      console.error('❌ Error en bloquearCuenta:', error);
      throw error;
    }
  }

  // 🔐 SEGURIDAD: Resetear intentos tras login exitoso
