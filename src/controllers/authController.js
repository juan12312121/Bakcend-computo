...
exports.login = async (req, res) => {
  try {
    ...
    if (usuario.bloqueo_hasta && new Date(usuario.bloqueo_hasta) > new Date()) {
      await logSecurityEvent(usuario.id, 'LOGIN_BLOCKED', 'Intento de login en cuenta bloqueada', req);
      return errorResponse(res, `Cuenta bloqueada temporalmente. Intenta de nuevo después de las ${new Date(usuario.bloqueo_hasta).toLocaleTimeString()}`, 403);
    }
    ...
    if (!contrasenaValida) {
      ...
      await Usuario.registrarIntentoFallido(usuario.id);

      // Bloquear si llega a 5
      if ((usuario.intentos_fallidos + 1) >= 5) {
        await Usuario.bloquearCuenta(usuario.id);
        await logSecurityEvent(usuario.id, 'ACCOUNT_LOCKED', 'Cuenta bloqueada por exceso de intentos', req);
        return errorResponse(res, 'Demasiados intentos fallidos. Cuenta bloqueada por 30 minutos.', 403);
      }

      await logSecurityEvent(usuario.id, 'LOGIN_FAILED', 'Contraseña incorrecta', req);
      return errorResponse(res, 'Credenciales inválidas', 401);
    }
    ...
    await logSecurityEvent(usuario.id, 'LOGIN_SUCCESS', 'Login exitoso', req);
